...
# | 2017 by Rafael Hernamperez             |
# | Example code to demonstrate the        |
# | Breadth First Search (BFS),            |
# | Depth First Search (DFS), and          |
# | A-Star (A*) shortest search algorithms |
# + ---------------------------------------+
# Examples of execution:
#    python driver.py bfs 1,2,0,5,3,4,8,6,7
#    python driver.py dfs 3,0,1,2,4,5,6,7,8
#    python driver.py ast 1,2,3,4,0,5,6,7,8
# Test Cases:
#    - Test Case #1
...
method = ""         # Method parameter: bfs, dfs or ast
board = ""          # Board parameter: 0,1,2,3,4,5,6,7,8
goal = "012345678"  # Goal to be reached by the board
...
def compare_nodes(x, y):
    return (x.depth + heuristic(x.state, goal)) - (y.depth + heuristic(x.state, goal))
...
# Main method
def execute():
    # def execute(method, start):
    # Configure the search
    # Example: Start: 120534867 - goal=012345678

    print("Executing " + method + " agent. This operation could take a long time. Please wait...")

    starting_time = datetime.datetime.now()
    path_to_goal = None

    if method == "bfs":
        path_to_goal = bfs_agent(board, goal)
    elif method == "dfs":
        path_to_goal = dfs_agent(board, goal)
    elif method == "ast":
        path_to_goal = a_star_agent(board, goal)
...
# Method: print_syntax()
# Goal: Print the syntax command when it is needed
def print_syntax():
    print("Syntax: \npython driver.py <method> <board>")
    print("   - <method>: bfs, dfs or ast")
    print("   - <board>: list of 9 numbers from 0 to 8 (0,1,2,3,4,5,6,7,8)")
...
if len(sys.argv) != 3:
    ...
    print("Executing driver.py...\n")
    method = sys.argv[1].lower()
    print("Method: " + method)

    # Check the first parameter (<method>)
    if method != "bfs" and method != "dfs" and method != "ast":
        ...
        # Omit the commas in order to create a string
        # with the right board format
        for ch in board:
            if ch != ',':
                tmp = tmp + ch
        ...
        if len(tmp) != 9:
            print("\n*** SYNTAX ERROR ***\nThe <board> parameter must have exactly 9 values\n")
            print_syntax()
        else:
            board = tmp
            is_error = False

            # Check if each value in the board is between 0 and 8
            for ch in tmp:
                if ch not in ['0', '1', '2', '3', '4', '5', '6', '7', '8']:
                    is_error = True
                    break

            if is_error:
                print("\n*** SYNTAX ERROR ***\nThe <board> parameter must be a value between 0 and 8\n")
                print_syntax()
            ...
            if is_error:
                print("\n*** SYNTAX ERROR ***\nThe <board> parameter have a repeated value\n")
                print_syntax()
            else:
